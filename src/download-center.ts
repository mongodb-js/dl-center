...
type S3UploadFunc = (req: S3.PutObjectRequest) => Promise<S3.ManagedUpload.SendData>;
type S3GetObjectFunc = (params: S3.GetObjectRequest) => Promise<S3.GetObjectOutput>;

type DownloadCenterConfigPlatform = {
  [key: string]: any;
...
export class DownloadCenter {
  ...
  async uploadAsset(
    s3ObjectKey: string,
    content: Content,
    options: UploadAssetOptions = {}): Promise<void> {
    if (!s3ObjectKey) {
      throw new Error('s3ObjectKey is required');
    }
